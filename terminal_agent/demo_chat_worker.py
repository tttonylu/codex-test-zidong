"""Quick verification script for the real chat worker BitBrowser call path."""
...
class MockBitBrowserHandler(BaseHTTPRequestHandler):
    """Mock BitBrowser endpoints used by the chat worker."""
        ...
        if self.path == "/browser/open":
            body = {
                ...
                "data": {
                    ...
                    "ws": "ws://127.0.0.1:53325/devtools/browser/mock-chat",
...
def main() -> None:
    server = ThreadingHTTPServer(("127.0.0.1", 15440), MockBitBrowserHandler)
    ...
    try:
        ...
        execution = registry.execute(
            TaskAssignmentPayload(
                task_id="task-chat-01",
                terminal_id="terminal-chat-01",
                instance_id="browser-chat-01",
                script_name="chat",
                parameters={"target_handle": "target_user"},
                priority=1,
            ),
            terminal_hostname="workstation-chat",
